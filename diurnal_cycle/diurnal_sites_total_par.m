...
hold on
obs_clear = nanmean(par_total_obs(:,skyconditions == 0), 2);
epic_clear = nanmean(par_total_epic(:,skyconditions == 0), 2);
obs_cloudy = nanmean(par_total_obs(:,skyconditions == 1), 2);
epic_cloudy = nanmean(par_total_epic(:,skyconditions == 1), 2);
obs_overcast = nanmean(par_total_obs(:,skyconditions == 2), 2);
epic_overcast = nanmean(par_total_epic(:,skyconditions == 2), 2);
...
xlim([1 24])
ylim([0 600])
text(1.2, 540, ['RRMSE_{clear}:' num2str(RRMSE_clear*100,'%.2f') '%'], 'fontsize', 8,'fontweight','bold')
text(1.2, 480, ['RRMSE_{cloudy}:' num2str(RRMSE_cloudy*100,'%.2f') '%'], 'fontsize', 8, 'fontweight','bold')
text(1.2, 420, ['RRMSE_{overcast}:' num2str(RRMSE_overcast*100,'%.2f') '%'], 'fontsize', 8, 'fontweight','bold')

title([serials{i} ' SURFRAD - ' sitenames{i}])
xlabel('Hour')
ylabel('Total PAR (W/m^2)')
box on
...
%% save file
print(gcf, '-dtiff', '-r600', 'total_par.tif')
